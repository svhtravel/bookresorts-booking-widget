...
<div id="brp-root">
    ...
    <!-- Departure City (shown only for Hotel + Flight) -->
...
if (checkin && checkout) {
  ...
  /* =========================
     ...
     ========================= */
  // If you later want to send leads to a webhook, put the URL here.
  // Leave as "" for now (it will just show success without sending anywhere).
  const LEAD_WEBHOOK_URL = "";

/* =========================
   HOTEL ID RESOLUTION (GLOBAL)
   ========================= */

// Will hold the resolved MakCorp Hotel ID
...
// Get WordPress site title (cleaned)
function getWordPressSiteTitle() {
  // 1️⃣ Rank Math / Yoast / SEO meta
  const ogSite = document.querySelector('meta[property="og:site_name"]');
  if (ogSite?.content) return ogSite.content.trim();

  // 2️⃣ WordPress global (if available)
  if (window._wpSiteName) return window._wpSiteName.trim();

  // 3️⃣ Header logo text (common in themes)
  const headerTitle =
    document.querySelector('.site-title, .logo-text, header h1');
  if (headerTitle?.textContent) return headerTitle.textContent.trim();

  // 4️⃣ LAST resort — document.title (fallback)
...
// Lookup hotel ID from public Google Sheet
...
// Resolve hotel ID once on page load
...
if (attempt === maxAttempts && normalized.length > 0) {
  return normalized;
    ...
    // wait before next attempt (except after last try)
...
return [];
  ...
  function formatRange(ci,co){
    const a=parseLocalDate(ci), b=parseLocalDate(co);
    const opt={month:"short",day:"numeric"};
    return `${a.toLocaleDateString("en-US",opt)}–${b.toLocaleDateString("en-US",opt)}, ${a.getFullYear()}`;
  }
  function closePopup(){
    if (window.elementorProFrontend && elementorProFrontend.modules && elementorProFrontend.modules.popup) {
      try { elementorProFrontend.modules.popup.closePopup(); return; } catch(e){}
    }
    const btns=document.querySelectorAll('.dialog-close-button, .elementor-popup-modal .eicon-close,[aria-label="Close"]');
  ...
  v.vendor.toLowerCase() === p.toLowerCase()
);
...
function calculateOurRate(results, discountPct = 2.5) {
  ...
  // Only consider Expedia & Hotels.com
  const eligible = results.filter(r =>
    ["Expedia.com", "Hotels.com"].includes(r.vendor)
  );
  ...
  const cheapest = eligible.reduce((a, b) =>
    parseMoney(a.total) <= parseMoney(b.total) ? a : b
  );
...
function renderComparisonResults(results, name, phone){
  ...
  return `
    <div style="width:100%;max-width:600px;">
      ...
      <!-- OUR RATE -->
      <div style="
        border:2px solid #b89b47;
        border-radius:18px;
        padding:20px;
        margin-bottom:22px;
        background:linear-gradient(135deg,#fff8e6,#ffffff);
        box-shadow:0 10px 30px rgba(184,155,71,.25);
      ">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div>
            <div style="font-size:14px;color:#555;">
              ${ourRate.nightly} / night
            </div>
            <div style="font-size:13px;color:#1f7a3f;font-weight:800;">
              You save ${ourRate.savings}
            </div>
          </div>
        ...
        <div style="margin-top:12px;font-size:13px;color:#555;">
          ✔ No hidden fees &nbsp; ✔ Dedicated resort specialist
        </div>
      </div>
      ...
      ${results.map(r=>`
        ...
        ">
          <div style="display:flex;align-items:center;gap:14px;">
            <img src="${VENDOR_LOGOS[r.vendor] || ''}"
                 style="height:22px;max-width:110px;object-fit:contain">
            <div>
  ...
  A ${hotelName} specialist will be reaching out to you soon.
</div>


  <div id="phone-confirmation-area" style="margin-top:12px; text-align:center;">
  <div id="confirm-phone-display" style="font-size:16px; margin-bottom:6px;">
    📞 <b>${phone}</b>
  </div>
  ...
  <div style="margin-top:10px; display:flex; gap:10px; justify-content:center;">
    <button id="phone-yes-btn"
      style="padding:8px 16px; border-radius:8px; border:none; background:#1f7a3f; color:#fff; font-weight:700; cursor:pointer;">
      Yes
    </button>

    <button id="phone-edit-btn"
      style="padding:8px 16px; border-radius:8px; border:1px solid #ccc; background:#fff; font-weight:700; cursor:pointer;">
      Update Number
    </button>
  </div>

  <!-- UPDATE NUMBER (same input style as lead form) -->
...
></select>
  ...
  <div style="margin-top:10px; text-align:center;">
    <button id="phone-save-btn"
      style="padding:8px 18px; border-radius:8px; border:none; background:#1a73e8; color:#fff; font-weight:800; cursor:pointer;">
      Save
    </button>
  </div>
</div>
 ...
 <div style="margin-top:14px; text-align:center;">
  <div style="
    ...
    color:#8a6d1d;
  ">
    ⏱ Average wait time: <b>Less than 1 min</b>
  </div>
</div>
...
if (countrySelect && phoneInput) {
 ...
 // Pin United States first, then sort others alphabetically
const us = COUNTRIES.find(c => c.label.includes("United States"));
const others = COUNTRIES
  .filter(c => !c.label.includes("United States"))
...
countrySelect.innerHTML = SORTED_COUNTRIES.map(c =>
  `<option
     value="${c.code}"
     data-full="${c.label} ${c.code}"
     data-short="${c.label.split(' ')[0]} ${c.code}"
   >
     ${c.label} ${c.code}
   </option>`
).join("");
...
countrySelect.addEventListener("change", updateSelectedCountryText);
  ...
  // Temporarily swap label for collapsed view
...
// Update on change
countrySelect.addEventListener("change", updateSelectedCountryLabel);
...
phoneInput.addEventListener("input", () => {
  if (countrySelect.value === "+1") {
    phoneInput.value = formatPhoneUS(phoneInput.value);
  }
});
...
container.dataset.leadName = name;
  ...
  <div class="brp-lead-wrap">
    ...
    <div class="brp-success-sub">
      A ${getWordPressSiteTitle()} specialist will be reaching out shortly.
    </div>
    ...
    ">
      ...
      <div style="
        ...
        border:1px solid rgba(0,0,0,.08);
      ">
        <div style="font-size:12px;color:#777;font-weight:700;letter-spacing:.06em;">
          TRAVEL DATES
        </div>
        <div style="font-size:17px;font-weight:800;margin-top:4px;">
          ${lockedBooking.checkIn} → ${lockedBooking.checkOut}
        </div>
      </div>
      ...
      ">
        ...
        <div style="font-size:16px;font-weight:700;margin-top:6px;line-height:1.5;">
          ${roomsConfig}
        </div>
      </div>
    ...
    <div id="phone-confirmation-area" style="margin-top:22px;text-align:center;">
      ...
      <div id="confirm-phone-display" style="font-size:18px;margin-bottom:6px;">
        📞 <b>${phone}</b>
      </div>
      ...
      <div style="margin-top:12px;display:flex;gap:10px;justify-content:center;">
        <button id="phone-yes-btn"
          style="padding:8px 18px;border-radius:8px;border:none;background:#1f7a3f;color:#fff;font-weight:700;">
          Yes
        </button>

        <button id="phone-edit-btn"
          style="padding:8px 18px;border-radius:8px;border:1px solid #ccc;background:#fff;font-weight:700;">
          Update Number
        </button>
      </div>
      ...
      <div id="phone-edit-wrap" style="display:none;margin-top:14px;">
        ...
        <div style="margin-top:10px;">
          <button id="phone-save-btn"
            style="padding:8px 18px;border-radius:8px;border:none;background:#1a73e8;color:#fff;font-weight:800;">
            Save
          </button>
        </div>
      </div>
...
lockedBooking = JSON.parse(container.dataset.pendingBooking || "{}");
  ...
  // SAME COUNTRY LIST AS HOTEL ONLY
  const COUNTRIES = [
    { code: "+1",  label: "🇺🇸 United States" },
    { code: "+44", label: "🇬🇧 United Kingdom" },
    { code: "+61", label: "🇦🇺 Australia" },
  ...
  yesBtn.onclick = () => {
    confirmArea.innerHTML = `
      <div style="font-size:15px;font-weight:700;color:#1f7a3f;">
        Thanks! Our team will connect with you shortly.
      </div>
    `;
  };
  ...
  // 🔁 Allow Zapier resend for phone update
...
const workerUrl =
  ...
  booking: lockedBooking
      ...
      try{
        ...
        // still show success (you can change this later)
...
if (!results || !results.length) {
  ...
  // ♻️ FULL INTERNAL RESET USING SAME LOGIC
  if (window.__BRP_RESET__) {
    window.__BRP_RESET__();
  }

  // 🔴 Show error after reset
  setTimeout(() => {
...
// =========================
// 🚀 FINAL ZAPIER WEBHOOK (CORS-SAFE)
// =========================
...
setTimeout(() => {
  ...
  const blob = new Blob(
    [JSON.stringify(payload)],
    { type: "text/plain" } // 🔑 MUST be text/plain
  );
...
/* 👇 PASTE THIS RIGHT HERE — NOT ABOVE, NOT BELOW */
...
setTimeout(() => {
  ...
  /* 🌍 SAME COUNTRY LIST AS LEAD FORM */
  const COUNTRIES = [
    { code: "+1",  label: "🇺🇸 United States" },
    { code: "+44", label: "🇬🇧 United Kingdom" },
    { code: "+61", label: "🇦🇺 Australia" },
  ...
  rooms.forEach((r, idx) => {
    ...
    let adultsHtml = `
      <div class="rg-ctrl">
        ...
        <div class="rg-steps">
          <button class="rg-step" type="button"
            data-op="minus" data-room="${idx}" data-field="adults">−</button>
          <span class="rg-count" id="count-adults-${idx}">${r.adults}</span>
          <button class="rg-step" type="button"
            data-op="plus" data-room="${idx}" data-field="adults">＋</button>
        </div>
      </div>
    `;
    ...
    if (ACTIVE_MAX_CHILDREN > 0) {
      childrenHtml = `
        <div class="rg-ctrl">
          ...
          <div class="rg-steps">
            <button class="rg-step" type="button"
              data-op="minus" data-room="${idx}" data-field="children">−</button>
            <span class="rg-count" id="count-children-${idx}">${r.children}</span>
            <button class="rg-step" type="button"
              data-op="plus" data-room="${idx}" data-field="children">＋</button>
          </div>
        </div>
      `;
    }
...
summary.addEventListener('click',()=>{
  ...
  const max =
    field === "adults" ? ACTIVE_MAX_ADULTS : ACTIVE_MAX_CHILDREN;
  ...
  /* =========================
     Main button: validate -> replace UI
     ========================= */
  const btn = q("#brp-check-btn");
  btn.addEventListener("click", (e)=>{
    e.preventDefault();

// Clear no-rates error safely (error lives OUTSIDE popup)
...
if (errorBox) {
  errorBox.style.display = "none";
    ...
    // swap popup into spinner + lead form
    showLoadingLeadForm({
      bookingType: type,
