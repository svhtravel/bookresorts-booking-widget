...
(function() {
  ...
  function initBookingPopup() {
    ...
    function getType(){
      const checked = q('input[name="booking_type"]:checked');
    ...
    function renderRooms(){
      ...
      rooms.forEach((r, idx) => {
        ...
        room.innerHTML = `
          ...
          ${adultsHtml}
          ${childrenHtml}
          ${idx > 0 ? `<button data-remove="${idx}" type="button" style="background:none;border:none;color:#b33;font-weight:700;cursor:pointer;font-size:15px;margin-top:8px;">Remove Room</button>` : ''}
        `;
    ...
    if (checkin) checkin.addEventListener('click',()=>openPicker(checkin));
    if (checkout) checkout.addEventListener('click',()=>openPicker(checkout));
  ...
  (async function init() {
    ...
    window.__BR_OPEN_INTERNAL__ = function() {
    ...
    window.BR_WIDGET_READY = true;
    console.log('✅ Widget marked as ready');

    if (window.BR_WIDGET_QUEUE && window.BR_WIDGET_QUEUE.length) {
      console.log('📋 Processing queued calls:', window.BR_WIDGET_QUEUE.length);
      window.BR_WIDGET_QUEUE.forEach(() => {
        if (window.__BR_OPEN_INTERNAL__) window.__BR_OPEN_INTERNAL__();
      });
      window.BR_WIDGET_QUEUE = [];
    }
  })();
